# AGENTS.md

Read [`AI-REVIEWER.md`](AI-REVIEWER.md) first — it is written for you, and it sets the order.

1. Declare your branch: Read, Read + Run, Read + Browse, or Full (§0).
2. Follow that branch's numbered sequence (§3). Against every step your branch
   cannot reach, write "not executed — claimed by the documents, not verified by
   me". Do not hand your operator a runbook unasked.
3. Report in the shape §4 gives, so your review is comparable with others.

If you can browse, the live artefact's itinerary at
https://danielarussell.com/ai-reviewer.txt is a numbered step, not a footnote.
